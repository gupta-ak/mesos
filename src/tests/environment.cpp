...
private:
      ...
      const string& networkName)
  {
    ...
    return docker->run(
      ...
      .then([=](const Option<int>& status) -> Future<Nothing> {
        if (!status.isSome()) {
          return Failure("Container " + containerName +
                          " failed with unknown exit code");
        }

        if (status.get() != 0) {
          return Failure("Container " + containerName +
                          " returned exit code " + stringify(status.get()));
        }
  ...
  Option<Error> runNetNamespaceCheck(const Owned<Docker>& docker) {
    ...
    Future<Nothing> containers = launchContainer(docker, container1, "nat")
      .then(process::defer([=]() {
