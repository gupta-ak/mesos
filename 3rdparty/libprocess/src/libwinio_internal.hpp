...
#ifndef __LIBWINIO_INTERNAL_HPP__
#define __LIBWINIO_INTERNAL_HPP__
...
public:
  ...
  // Functions to register and unregister a handle for async IO.
  Try<Nothing> registerHandle(const int_fd& fd);
  Try<Nothing> unregisterHandle(const int_fd& fd);
...
// can be discarded to cancel the Future.
Future<size_t> read(
    const int_fd& fd,
    void* buf,
    size_t size);
...
#endif // __LIBWINIO_INTERNAL_HPP__
