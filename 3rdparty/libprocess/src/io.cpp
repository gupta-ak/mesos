...
Future<string> read(int_fd fd)
{
  ...
  if (async.isError()) {
    os::close(fd);
    return Failure("Failed to prepare fd for async IO: " + async.error());
  }
    ...
    const vector<lambda::function<void(const string&)>>& hooks)
{
  ...
  if (async.isError()) {
    ...
    os::close(to.get());
    return Failure(
        "Failed to prepare `from` fd for async IO: " + async.error());
  }
  ...
  if (async.isError()) {
    ...
    os::close(to.get());
    return Failure(
        "Failed to prepare 'to' fd for async IO: " + async.error());
  }
