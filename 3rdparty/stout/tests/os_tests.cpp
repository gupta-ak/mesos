...
{
  ASSERT_SOME_TRUE(os::isCloexec(INVALID_HANDLE_VALUE));
}
#else
TEST_F(OsTest, Cloexec)
{
...
{
  EXPECT_SOME_TRUE(os::isNonblock(INVALID_HANDLE_VALUE));
  EXPECT_SOME(os::nonblock(INVALID_HANDLE_VALUE));
  EXPECT_ERROR(os::nonblock(INVALID_SOCKET));
}
#else
TEST_F(OsTest, Nonblock)
{
