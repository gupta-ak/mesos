...
public:
  ...
  //
  // NOTE: The `key` parameter is `CompletionKey` in `CreateIoCompletionPort`.
  // As stated by the MSDN docs, this key is the "per-handle user-defined
  // completion key that is included in every I/O completion packet for the
  // specified file handle" [1], and thus, it's only used for bookeeping by the
  // user and not used for functional control in `CreateIoCompletionPort`.
  //
